...
ArduinoSerial = serial.Serial('COM3',2400) #COM3 is the port of Dispensing unit
time.sleep(2) 
...
def calibration(Z0, a_gcode, b_gcode, r_stage):
    ...
    while True:
        if keyboard.read_key()==('a'):
            a_gcode=a_gcode+stepsize
        if keyboard.read_key()==('d'):
            a_gcode=a_gcode-stepsize
        if keyboard.read_key()==('s'):
            b_gcode=b_gcode+stepsize
        ...
        if keyboard.read_key()==('f'):
            Z0=Z0-stepsize
        swift.set_position(a_gcode, b_gcode,Z0, wait=True)
        if keyboard.read_key()==('q'):
            break
        return a_gcode, b_gcode,Z0

# User-defined inputs
spp = 20 # Printing speed 
spnp = 2000  # Speed at non-printing paths
Z0 = 50 # Starting z position
...
stage = 2 # Petri dish '1': 35 mm, '2': 55 mm, '3': 90 mm, '4': 35 mm heating stage
          # Rectangular container '5': 33 mm, '6': 40 mm
          # '7': Standard glass slide

gcode_file = r"C:\Users\Biointerface\Documents\uArmPython\examples\Josh\testb.txt"# import gcode file
...
ab = [[280,-13]] #Approx. positions of printheads 1,2,3,4
stage_mega_r = [33, 44.5, 60, 36, 33, 38.2, 29.8]
a_gcode = ab[0][0] # x coord of the printhead from uArm base, mm 
b_gcode = ab[0][1] # y coord of the printhead from uArm base, mm
r_stage = stage_mega_r[stage-1]
x_t=[0]
y_t=[0]
xy_gcode = transform.coordinateTransfer(x_t, y_t, a_gcode, b_gcode, r_stage).T
swift.set_position(xy_gcode[0,0], xy_gcode[0,1],10, wait=True)
swift.set_position(xy_gcode[0,0], xy_gcode[0,1],Z0, wait=True)
stepsize=float(input(' what is the stepsize you want?'))
print("use wasd to control xy position, ef for Z position and q to quit")
