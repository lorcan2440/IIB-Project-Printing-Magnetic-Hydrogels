# -*- coding: utf-8 -*-
"""
Created on Mon Jun 21 19:19:49 2021
...
for i in range(0, 8):
    ...
    while k <= 1:
        if k == 0:
             x1 = np.linspace(1.5+p, 8.5+p, 100)
             y1 = A_eq*np.sin(B_eq*(x1 + C_eq)) + D_eq

             x2 = np.linspace(2.7+p, 9.7+p, 100)
        ...
        if k == 1:
             x1 = np.linspace(1.5-m+p, 8.5-m+p, 100)
             y1 = A_eq*np.sin(B_eq*(x1 + C_eq)) + D_eq

             x2 = np.linspace(2.7-m+p, 9.7-m+p, 100)
             y2 = -A_eq*np.sin(B_eq*(x2 + C_eq)) + D_eq
