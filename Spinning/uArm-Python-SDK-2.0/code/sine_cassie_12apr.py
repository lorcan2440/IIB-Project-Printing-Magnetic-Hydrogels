...
A_eq = 2.5 # Amplitude
period = 6
B_eq = 2*np.pi/period # Period = 2pi/period
C_eq = 3 # Phase shift, x axis
D_eq = 0 # Vertical shift
p = 0 # adjust the x shift 
m=11

sine_x = np.linspace(3, 15, 100) #print area is 18x18mm
sine_y = A_eq*np.sin(B_eq*(sine_x - C_eq)) + D_eq

#populate the 2D arrays that stores the positions that the arm needs to move to during printing
#sine
a_printpath = [*sine_y] #x positions in arm coord system
b_printpath = [*sine_x] #y positions in arm coord system
