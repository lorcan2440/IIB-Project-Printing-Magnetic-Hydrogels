...
# Circle coordinates
Turn = np.linspace(0,2.7*2*np.pi, 100)[:-1]   
...
swift.set_position(xyzarm_spi[0, 0]-5 , xyzarm_spi[0, 1], 10, speed=3000, timeout=20)
time.sleep(1)
#62
swift.set_position(xyzarm_spi[0, 0]-5 , xyzarm_spi[0, 1] , 59.6, wait=True)
isTerminate = input('Continue? Please type any key to continue [Y/n]: ')
if isTerminate == 'n':
    sys.exit()
...
time.sleep(3)
swift.set_position(xyzarm_spi[0, 0], xyzarm_spi[0, 1], 59.6, speed = 20, wait=True)
swift.flush_cmd(wait_stop=True)
#59.6
n = 0   
...
for x, y, z in xyzarm_spi:
        swift.set_position(x, y, z,speed = spp+(n/100)*5*spp, wait=True)
        n =  n+1
        swift.flush_cmd(wait_stop=True)
