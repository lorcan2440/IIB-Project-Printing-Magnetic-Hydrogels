...
x_spi = 0.9*R*np.cos(Turn)
y_spi = 0.9*R*np.sin(Turn)
z_spi = 0.8*(0.122*Turn**2-0.388*Turn-0.00917*Turn**3+(2.26*10**(-4))*Turn**4-2.55)
a_spi = 282.5  # mm
#282.5
...
time.sleep(1)
#62
swift.set_position(xyzarm_spi[0, 0] , xyzarm_spi[0, 1] , 58.1, wait=True)
...
#time.sleep(3)
#62
swift.set_position(xyzarm_spi[0, 0], xyzarm_spi[0, 1], 58.4, speed = 20, wait=True)
...
for x, y, z in xyzarm_spi:
        swift.set_position(x, y, z,speed = spp+(n/100)*15*spp, wait=True)
        n =  n+1
        swift.flush_cmd(wait_stop=True)
