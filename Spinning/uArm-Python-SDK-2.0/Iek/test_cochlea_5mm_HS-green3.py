...
# Circle coordinates
Turn = np.linspace(0,2.7*2*np.pi, 100)   
R = 0.09402*Turn**2-1.042*Turn-0.002899*Turn**3+5.745
x_spi = 0.89*R*np.cos(Turn)
y_spi = 0.89*R*np.sin(Turn)
z_spi = 1.23*(0.122*Turn**2-0.388*Turn-0.00917*Turn**3+(2.26*10**(-4))*Turn**4-2.55)
a_spi = 282.5  # mm
#282.5
b_spi =  2.5 # mm 2.5 old stage
#60
z0 = 61
spp = 30
#25 17800
...
swift.flush_cmd(wait_stop=True) #-2
swift.set_position(xyzarm_spi[0, 0] , xyzarm_spi[0, 1], 10, speed=3000, timeout=20)
time.sleep(2)
#62
swift.set_position(xyzarm_spi[0, 0] , xyzarm_spi[0, 1] , 64.1, wait=True) #62.4
...
time.sleep(1)
#62
swift.set_position(xyzarm_spi[0, 0], xyzarm_spi[0, 1], 64.1, speed = 20, wait=True)
...
for x, y, z in xyzarm_spi:
        swift.set_position(x, y, z,speed = spp+(n/100)*(0)*spp, wait=True)
        n =  n+1
        swift.flush_cmd(wait_stop=True)
