...
# Load Gcode
spp = 23  # Printing speed
spnp = 1000  # Not printing speed 
start_from = int(input('which sequence you want to start from?'))
k=start_from-1
List=[[2.5, 7], [2.5, 0], [2.5, -7], [-2.5, -7], [-2.5, 0], [-2.5, 7]]
while k<=5:
    offset_i=List[k]
    Z0 = 65.95
    while True:
        offset_i.append(Z0)
        printhead = 6 #printehead5 is in nano printhead3
        stage = 3 #1 Small, 2 Large
    ...
    # X-Y coordinates transformation
        ab = [[180.5,76], [253,-4], [280,-74], [279.5,-3], [254, 4.2]] #Calibrated printhead 2,3,4,5 a,b coordinates
    ...
    # Transfrom printing coordinates to uArm coordinates, same ref point.
        ...
        Zcheck = input('is the height OK? y/any key to change Z value ')
        if Zcheck == 'y':
            break
        del offset_i[2:3]
        Z0 = Z0+float(input('how much should the height increase'))
