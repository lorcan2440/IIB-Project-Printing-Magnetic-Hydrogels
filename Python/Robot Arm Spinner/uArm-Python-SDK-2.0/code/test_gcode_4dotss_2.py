...
a=-3.3 #x cordinate of the first point
b=-7 #y cordinate of the first point
k=0
for num, offset_i in enumerate([[a, b, Z0], [a-2.6, b+9.6, Z0], [a+9.8, b+2.6, Z0], [a+7.2, b+12.2, Z0]]):
    k=k+1
    bigCoor = gcodemod.read(r"C:\Users\Biointerface\Documents\uArmPython\examples\Yaqi\sbox.txt", offset_i) # Import gcode
    ...
    if k>=2:
        swift.set_position(z=50, speed=3000, wait=True)
        time.sleep(0)
    swift.flush_cmd(wait_stop=True)
    if num == 0:
        swift.set_position(xyarm_gcode[0, 0], xyarm_gcode[0, 1], 10, speed=3000, timeout=20)
    else: 
        swift.set_position(xyarm_gcode[0, 0], xyarm_gcode[0, 1], 50, speed=3000, timeout=20)
    ...
    if num == 0:
        ...
        isTerminate = input('Continue? Please type any key to continue [Y/n/c]: ')
        if isTerminate == 'n':
            sys.exit()
