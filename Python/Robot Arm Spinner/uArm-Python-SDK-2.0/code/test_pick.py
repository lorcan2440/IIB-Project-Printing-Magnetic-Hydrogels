...
if printhead==2:
    a=280.6
    b=70.5
x_cor=[6,6,-6,-6,6,6,-6,-6]
y_cor=[6,-6,-6,6,-34,-46,-46,-34]
...
swift.set_position(x=xyarm_cir[k][0]+da, y=xyarm_cir[k][1]+db, z=Z0-3, wait=True)
while True:
    checkpos = input('is the position right?(Y/ n)')
    if checkpos == 'n':
        da=da+float(input('horizental shift'))
        db=db+float(input('vertical shift'))
        swift.set_position(x=xyarm_cir[k][0]+da, y=xyarm_cir[k][1]+db, z=Z0-3, wait=True)
    else:
...
while k<=8:
    ...
    if printhead==2:
        ArduinoSerial.write(b'8') # Turn on printhead 8, sucking
    swift.set_position(x=xyarm_cir[k][0]+da+4, y=xyarm_cir[k][1]+db, z=Z0, wait=True)
    ...
    #swift.set_position(x=150, y=-2, z=10, speed=3000, wait=True)
    ...
    if isTerminate == 'n':
        ...
        swift.set_position(x=105, wait=True)
        sys.exit()
    swift.set_position(x=xyarm_cir[k+4][0]+dc, y=xyarm_cir[k+4][1]+dd, z=10, wait=True)
    swift.set_position(x=xyarm_cir[k+4][0]+dc, y=xyarm_cir[k+4][1]+dd, z=Z0-2, wait=True)
    while True:
        checkpos = input('is the position right?(Y/ n)')
        if checkpos == 'n':
            dc=dc+float(input('horizental shift'))
            dd=dd+float(input('vertical shift'))
            swift.set_position(x=xyarm_cir[k+4][0]+dc, y=xyarm_cir[k+4][1]+dd, z=10, wait=True)
            swift.set_position(x=xyarm_cir[k+4][0]+dc, y=xyarm_cir[k+4][1]+dd, z=Z0-2, wait=True)
        else:
            break
    ...
    ArduinoSerial.write(b'5')
    k=k+1
    print('WAIT!!!! sleeping before moving up!!!!')
    time.sleep(3)
    swift.set_position(x=xyarm_cir[k][0]+da, y=xyarm_cir[k][1]+db, z=10, wait=True)
    swift.set_position(x=xyarm_cir[k][0]+da, y=xyarm_cir[k][1]+db, z=Z0-3, wait=True)
    while True:
        checkpos = input('is the position right?(Y/ n)')
        if checkpos == 'n':
