...
R = 0.007049*Turn**2-0.3603*Turn+0.0002574*Turn**3+4.586
x_spi = R*np.cos(Turn)
y_spi = R*np.sin(Turn)
...
swift.set_position(xyzarm_spi[0, 0]-2 , xyzarm_spi[0, 1], 10, speed=3000, timeout=20)
time.sleep(1)
#62
swift.set_position(xyzarm_spi[0, 0]-2 , xyzarm_spi[0, 1] , 62.5, wait=True)
isTerminate = input('Continue? Please type any key to continue [Y/n]: ')
if isTerminate == 'n':
    sys.exit()
...
time.sleep(3)
swift.set_position(xyzarm_spi[0, 0], xyzarm_spi[0, 1], 62.5, speed = 20, wait=True)
swift.flush_cmd(wait_stop=True)
#59.6
n = 0   
