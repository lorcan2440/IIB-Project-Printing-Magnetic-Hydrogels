...
Z0 = 66 # starting z position 66.2 55 4.8 63.5 49.8
offset = [0, 0, Z0] # Setting as [0,0,Z0] means printing at the central point (a_gcode,b_gcde) of the stage at Z0
bigCoor = gcodemod.read(r"C:\Users\Biointerface\Documents\uArmPython\examples\Iek\Letter_C_13x16x2mm-60R_4s.txt", offset) # Import gcode
...
a_gcode = 282.5  # x coord of the printhead from uArm base, mm 2 284.5 left corner
b_gcode = 1.5 # y coord of the printhead from uArm base, mm 2left corner 1.5 right corner 3.5
...
for n, etem in enumerate(e_gcode):
    ...
    if zarm_gcode[n] != last_zarm:
        layer_count += 1
        last_zarm = zarm_gcode[n]
        if layer_count == n_layer_pause:
            # reset layer_count
            layer_count = 0
            # pause to do stuff
            ArduinoSerial.write(b'5')
            swift.set_position(xyarm_gcode[n, 0]-3, xyarm_gcode[n, 1]-3, zarm_gcode[n], speed=1000, wait=True)
            time.sleep(10)
            swift.set_position(xyarm_gcode[n, 0], xyarm_gcode[n, 1], zarm_gcode[n], speed=spp, wait=True)
