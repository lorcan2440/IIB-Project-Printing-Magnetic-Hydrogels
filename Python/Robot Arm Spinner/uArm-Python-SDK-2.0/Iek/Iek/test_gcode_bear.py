...
Z0 = 63  # starting z position 66.2 55 4.8
offset = [0, 0, Z0] # Setting as [0,0,Z0] means printing at the central point (a_gcode,b_gcde) of the stage at Z0
bigCoor = gcodemod.read(r"C:\Users\Biointerface\Documents\uArmPython\examples\Iek\gecko-90R-0_3s.txt", offset) # Import gcode
...
for n, etem in enumerate(e_gcode):
    ...
    if zarm_gcode[n] != last_zarm:
        ...
        if layer_count == n_layer_pause:
            ...
            if isTerminate == 'n':
                sys.exit()
            swift.set_position(xyarm_gcode[n, 0], xyarm_gcode[n, 1], 10, wait=True)
            swift.set_position(xyarm_gcode[n, 0], xyarm_gcode[n, 1], zarm_gcode[n], wait=True)
